...
string_view mm::AnnotationTypeToName(PdfAnnotationType type)
{
    switch (type)
    {
...
PdfAnnotationType mm::NameToAnnotationType(const string_view& str)
{
    if (str == "Text"sv)
...
void utls::RecursionGuard::Enter()
{
    ...
    if (s_recursionDepth > s_MaxRecursionDepth)
    {
        ...
        PDFMM_RAISE_ERROR_INFO(PdfErrorCode::InvalidXRef, "Stack overflow");
    }
}

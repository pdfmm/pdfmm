...
bool mm::IsPdfDocEncodingCoincidentToUTF8(const string_view& view)
{
...
string mm::ConvertUTF8ToPdfDocEncoding(const string_view& view)
{
    string ret;
    if (!TryConvertUTF8ToPdfDocEncoding(view, ret))
        PDFMM_RAISE_ERROR_INFO(PdfErrorCode::InvalidHandle, "Unsupported chars in converting utf-8 string to PdfDocEncoding");

    return ret;
}

bool mm::TryConvertUTF8ToPdfDocEncoding(const string_view& view, string& pdfdocencstr)
{
    auto& map = getUTF8ToPdfEncodingMap();

...
PdfObject* PdfNameTree::GetKeyValue(PdfObject& obj, const PdfString& key) const
{
    ...
    if (obj.GetDictionary().HasKey("Kids"))
    {
        ...
        for (auto& child : kids)
        {
            ...
            if (childObj == nullptr)
            {
                mm::LogMessage(PdfLogSeverity::Debug, "Object {} {} R is child of nametree but was not found!",
...
PdfNameLimits PdfNameTree::CheckLimits(const PdfObject& obj, const PdfString& key)
{
    ...
    {
        mm::LogMessage(PdfLogSeverity::Debug, "Name tree object {} {} R does not have a limits key!",

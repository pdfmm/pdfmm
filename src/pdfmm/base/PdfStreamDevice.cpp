...
size_t StreamDevice::SeekPosition(size_t curpos, size_t devlen, ssize_t offset, SeekDirection direction)
{
    switch (direction)
    {
        ...
        {
            if (offset > 0)
                PDFMM_RAISE_ERROR_INFO(PdfErrorCode::InvalidDeviceOperation, "Invalid positive seek");
            else if ((size_t)-offset > devlen)
                PDFMM_RAISE_ERROR_INFO(PdfErrorCode::ValueOutOfRange, "Invalid seek out of bounds");
...
size_t StandardStreamDevice::GetLength() const
{
    ...
    switch (GetAccess())
    {
        ...
        {
            PDFMM_INVARIANT(m_istream != nullptr);
...
fstream* FileStreamDevice::getFileStream(const string_view& filename, FileMode mode, DeviceAccess access)
{
    switch (mode)
    {
        case FileMode::CreateNew:
            if (access == DeviceAccess::Read)
                PDFMM_RAISE_ERROR_INFO(PdfErrorCode::InvalidDeviceOperation, "Invalid combination FileMode::CreateNew and DeviceAccess::Read");
            break;
        case FileMode::Create:
            if (access == DeviceAccess::Read)
                PDFMM_RAISE_ERROR_INFO(PdfErrorCode::InvalidDeviceOperation, "Invalid combination FileMode::Create and DeviceAccess::Read");
            break;
        case FileMode::OpenOrCreate:
            if (access == DeviceAccess::Read)
                PDFMM_RAISE_ERROR_INFO(PdfErrorCode::InvalidDeviceOperation, "Invalid combination FileMode::OpenOrCreate and DeviceAccess::Read");
            break;
        case FileMode::Truncate:
            if (access == DeviceAccess::Read)
                PDFMM_RAISE_ERROR_INFO(PdfErrorCode::InvalidDeviceOperation, "Invalid combination FileMode::Truncate and DeviceAccess::Read");
            break;
    ...
    switch (mode)
    {
        case FileMode::CreateNew:
            if (fs::exists(filename))
                PDFMM_RAISE_ERROR_INFO(PdfErrorCode::InvalidDeviceOperation, "The file must not exist");
            break;
        case FileMode::Create:
        ...
        case FileMode::Open:
            if ((access & DeviceAccess::Write) != DeviceAccess{ } && !fs::exists(filename))
                PDFMM_RAISE_ERROR_INFO(PdfErrorCode::InvalidDeviceOperation, "The file must exist");
            break;
        case FileMode::OpenOrCreate:
            break;
        case FileMode::Truncate:
    ...
    {
        delete stream;
        PDFMM_RAISE_ERROR_INFO(PdfErrorCode::InvalidDeviceOperation, filename);
    }

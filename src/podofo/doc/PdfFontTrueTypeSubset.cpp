...
void PdfFontTrueTypeSubset::LoadGlyphs(GlyphContext& ctx, const CIDToGIDMap& usedCodes)
{
    ...
    for (auto& pair : usedCodes)
    {
        if ((pair.first - prevCID) != 1)
            PODOFO_RAISE_ERROR_INFO(EPdfError::ValueOutOfRange, "The cid to gid map should be starting with 1 have consecutive indices");
    ...
    for (unsigned i = 0; i < m_Tables.size(); i++)
    {
        ...
        switch (table.Tag)
        {
            ...
            case TTAG_prep:
                ...
                break;
            default:
                PODOFO_RAISE_ERROR_INFO(EPdfError::InvalidEnumValue, "Unsupported table at this context");
        }

...
{
    PODOFO_RAISE_LOGIC_IF(!m_Append, "EndAppend() failed because BeginAppend() was not yet called!");
    endAppend();
}
...
{
    PODOFO_RAISE_LOGIC_IF(!m_Append, "Append() failed because BeginAppend() was not yet called!");
    if (len == 0)
        return *this;

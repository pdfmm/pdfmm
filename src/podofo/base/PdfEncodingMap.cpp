    ...
    const string_view::iterator& end, PdfCID& cid) const
{
    ...
    else
    {
        ...
        unsigned code = 0;
        unsigned char i = 1;
        PODOFO_ASSERT(i <= m_limits.MaxCodeSize);
        while (curr != end)
        {
...
{
    if (map == nullptr)
        PODOFO_RAISE_ERROR_INFO(EPdfError::InvalidHandle, "Map must be not null");
}
...
void PdfEncodingMapSimple::appendBaseFontEntries(PdfStream& stream) const
{
    ...
    for (; code < lastCode; code++)
    {
        if (!TryGetCodePoints(PdfCharCode(code), codePoints))
            PODOFO_RAISE_ERROR_INFO(EPdfError::InvalidFontFile, "Unable to find character code");
...
{
    PODOFO_RAISE_ERROR_INFO(EPdfError::NotImplemented, "PdfDynamicEncoding can't be used only from a PdfFont")
}

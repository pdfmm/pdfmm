...
#include <pdfmm/base/PdfParser.h>
#include <pdfmm/base/PdfXRefStreamParserObject.h>
...
constexpr long maxNumberOfIndirectObjects = 8388607;
...
namespace mm
{
    ...
    {
    public:
        PdfParserTest(PdfIndirectObjectList& objectList, const char* buffer, size_t len)
            : PdfParser(objectList), m_device(new PdfInputDevice(buffer, len))
        { }
        ...
        void ReadXRefContents(bool positionAtEnd, size_t offset)
        {
            // call protected method
            PdfParser::ReadXRefContents(m_device, positionAtEnd, offset);
        }
...
TEST(ParserTest, testMaxObjectCount)
{
    const long defaultObjectCount = PdfParser::GetMaxObjectCount();
    ...
    PdfParser::SetMaxObjectCount(numeric_limits<long>::max());
    ...
    PdfParser::SetMaxObjectCount(numeric_limits<short>::max());
    testReadXRefSubsection();
    testReadDocumentStructure();

    PdfParser::SetMaxObjectCount(numeric_limits<int>::max());
    testReadXRefSubsection();
    testReadDocumentStructure();
...
TEST(ParserTest, testReadDocumentStructure)
{
    // TODO no tests yet - stub method needed by testMaxObjectCount
}

TEST(ParserTest, testReadXRefContents)
{
    try
    {
        ...
        PdfParserTest parser(&objects, oss.str().c_str(), oss.str().length());
        parser.SetupTrailer();
    ...
    {
        CPPUNIT_ASSERT(error.GetError() == ePdfError_InvalidXRef);
    }
    catch (exception& ex)
    {
        CPPUNIT_FAIL("Unexpected exception type");
...
TEST(ParserTest, testReadXRefSubsection)
{
    ...
    if (PdfParser::GetMaxObjectCount() <= maxNumberOfIndirectObjects)
    {
        try
        {
            string strInputStream = generateXRefEntries(PdfParser::GetMaxObjectCount());
            PdfIndirectObjectList objects;
            PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
            nFirstObject = 0;
            nNumObjects = PdfParser::GetMaxObjectCount();
    ...
    if (PdfParser::GetMaxObjectCount() < numeric_limits<long>::max())
    {
        // don't generate xrefs for high values of GetMaxObjectCount() e.g. don't try to generate 2**63 xrefs
        size_t numXRefEntries = min(maxNumberOfIndirectObjects + 1, PdfParser::GetMaxObjectCount() + 1);

        try
        {
            string strInputStream = generateXRefEntries(numXRefEntries);
            PdfIndirectObjectList objects;
            PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
            nFirstObject = 0;
            nNumObjects = PdfParser::GetMaxObjectCount() + 1;
            parser.ReadXRefSubsection(nFirstObject, nNumObjects);
    ...
    try
    {
        ...
        PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
        nFirstObject = 1;
        nNumObjects = numeric_limits<size_t>::max() / 2 - 1;
        parser.ReadXRefSubsection(nFirstObject, nNumObjects);
        CPPUNIT_FAIL("PdfError not thrown");
    }
    catch (PdfError& error)
    {
        ...
        CPPUNIT_ASSERT(error.GetError() == ePdfError_InvalidXRef || error.GetError() == ePdfError_ValueOutOfRange || error.GetError() == ePdfError_OutOfMemory);
    }
    catch (exception& ex)
    {
        CPPUNIT_FAIL("Wrong exception type");
    ...
    {
        size_t maxObjects = numeric_limits<size_t>::max() / sizeof(PdfParser::TXRefEntry) / 100 * 95;
        ...
        catch (PdfError& error)
        {
            if (maxObjects >= (size_t)PdfParser::GetMaxObjectCount())
                ...
                CPPUNIT_ASSERT(error.GetError() == ePdfError_OutOfMemory);
        }
        catch (exception& ex)
        {
            CPPUNIT_FAIL("Wrong exception type");
    ...
    try
    {
        string strInputStream = "0000000000 65535 f\r\n";
        PdfIndirectObjectList objects;
        PdfParserTestWrapper parser(objects, strInputStream.c_str(), strInputStream.length());
        nFirstObject = -5LL;
        nNumObjects = 5;
    ...
    {
        CPPUNIT_ASSERT(error.GetError() == ePdfError_ValueOutOfRange || error.GetError() == ePdfError_NoXRef);
    }
    catch (exception& ex)
    {
        CPPUNIT_FAIL("Wrong exception type");
    ...
    try
    {
        ...
        PdfIndirectObjectList objects;
        PdfParserTestWrapper parser(objects, strInputStream.c_str(), strInputStream.length());
        nFirstObject = numeric_limits<long>::max();
        nNumObjects = numeric_limits<long>::max();
        parser.ReadXRefSubsection(nFirstObject, nNumObjects);
        CPPUNIT_FAIL("PdfError not thrown");
    }
    catch (PdfError& error)
    ...
    try
    {
        ...
        PdfIndirectObjectList objects;
        PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
        nFirstObject = numeric_limits<int64_t>::max();
        nNumObjects = numeric_limits<int64_t>::max();
        parser.ReadXRefSubsection(nFirstObject, nNumObjects);
        CPPUNIT_FAIL("PdfError not thrown");
    }
    catch (PdfError& error)
    ...
    CPPUNIT_ASSERT(PdfParser::GetMaxObjectCount() <= numeric_limits<long>::max());
    ...
    {
        CPPUNIT_ASSERT(error.GetError() == ePdfError_ValueOutOfRange);
    }
    catch (exception& ex)
    {
        CPPUNIT_FAIL("Wrong exception type");
    ...
    try
    {
        ...
        PdfIndirectObjectList objects;
        PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
        nFirstObject = numeric_limits<long>::min();
        nNumObjects = 1;
        parser.ReadXRefSubsection(nFirstObject, nNumObjects);
        CPPUNIT_FAIL("PdfError not thrown");
    }
    ...
    try
    {
        ...
        PdfIndirectObjectList objects;
        PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
        nFirstObject = numeric_limits<int64_t>::min();
        nNumObjects = 1;
        parser.ReadXRefSubsection(nFirstObject, nNumObjects);
        CPPUNIT_FAIL("PdfError not thrown");
    }
    ...
    try
    {
        ...
        PdfIndirectObjectList objects;
        PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
        nFirstObject = numeric_limits<size_t>::max();
        nNumObjects = 1;
        parser.ReadXRefSubsection(nFirstObject, nNumObjects);
        CPPUNIT_FAIL("PdfError not thrown");
    }
    catch (PdfError& error)
    {
        ...
        CPPUNIT_ASSERT(error.GetError() == ePdfError_ValueOutOfRange || sizeof(size_t) == 4);
        CPPUNIT_ASSERT(error.GetError() == ePdfError_InvalidXRef || sizeof(size_t) == 8);
    }
    catch (exception& ex)
    {
        CPPUNIT_FAIL("Wrong exception type");
    ...
    try
    {
        ...
        PdfIndirectObjectList objects;
        PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
        CPPUNIT_ASSERT(PdfParser::GetMaxObjectCount() > 0);
        nFirstObject = PdfParser::GetMaxObjectCount();
        nNumObjects = 1;
        parser.ReadXRefSubsection(nFirstObject, nNumObjects);
        CPPUNIT_FAIL("PdfError not thrown");
    }
    ...
    try
    {
        ...
        PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
        nFirstObject = 1;
        nNumObjects = numeric_limits<long>::min();
        parser.ReadXRefSubsection(nFirstObject, nNumObjects);
        CPPUNIT_FAIL("PdfError not thrown");
    }
    catch (PdfError& error)
    ...
    try
    {
        ...
        PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
        nFirstObject = 1;
        nNumObjects = numeric_limits<int64_t>::min();
        parser.ReadXRefSubsection(nFirstObject, nNumObjects);
        CPPUNIT_FAIL("PdfError not thrown");
    }
    catch (PdfError& error)
    ...
    try
    {
        ...
        PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
        nFirstObject = 1;
        nNumObjects = numeric_limits<size_t>::max();
        parser.ReadXRefSubsection(nFirstObject, nNumObjects);
        CPPUNIT_FAIL("PdfError not thrown");
    }
    catch (PdfError& error)
    ...
    for (int i = 0; i < static_cast<int>(numValues); ++i)
    {
        for (size_t j = 0; j < numValues; ++j)
        {
            ...
            catch (PdfError& error)
            {
                ...
                CPPUNIT_ASSERT(error.GetError() == ePdfError_InvalidXRef || error.GetError() == ePdfError_ValueOutOfRange || error.GetError() == ePdfError_NoXRef || error.GetError() == ePdfError_OutOfMemory);
            }
            catch (exception& ex)
            {
...
TEST(ParserTest, testReadXRefStreamContents)
{
    // test valid stream
    try
    {
    ...
    try
    {
        ...
        PdfIndirectObjectList objects;
        PdfParser::TVecOffsets offsets;
        PdfParserTestWrapper parser(&objects, oss.str().c_str(), oss.str().length());

        PdfXRefStreamParserObject xrefStreamParser(objects, parser.GetDevice(),
    ...
    {
        CPPUNIT_ASSERT(error.GetError() == ePdfError_NoXRef || error.GetError() == ePdfError_InvalidXRefStream);
    }
    catch (exception& ex)
    {
        CPPUNIT_FAIL("Unexpected exception type");
    ...
    {
        CPPUNIT_ASSERT(error.GetError() == ePdfError_NoXRef);
    }
    catch (exception& ex)
    {
        CPPUNIT_FAIL("Unexpected exception type");
    ...
    {
        CPPUNIT_ASSERT(error.GetError() == ePdfError_InvalidXRefStream);
    }
    catch (exception& ex)
    {
        CPPUNIT_FAIL("Unexpected exception type");
    ...
    {
        CPPUNIT_ASSERT(error.GetError() == ePdfError_InvalidXRefType);
    }
    catch (exception& ex)
    {
        CPPUNIT_FAIL("Unexpected exception type");
...
TEST(ParserTest, testReadObjects)
{
    ...
    {
        CPPUNIT_ASSERT(error.GetError() == ePdfError_InvalidEncryptionDict);
    }
    catch (exception& ex)
    {
        CPPUNIT_FAIL("Unexpected exception type");
...
TEST(ParserTest, testIsPdfFile)
{
    try
    {
        string strInputStream = "%PDF-1.0";
        PdfIndirectObjectList objects;
        PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
    ...
    try
    {
        string strInputStream = "%PDF-1.99";
        PdfIndirectObjectList objects;
        PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
        CPPUNIT_ASSERT(parser.IsPdfFile());
    }
    ...
    try
    {
        string strInputStream = "%!PS-Adobe-2.0";
        PdfIndirectObjectList objects;
        PdfParserTestWrapper parser(&objects, strInputStream.c_str(), strInputStream.length());
        CPPUNIT_ASSERT(!parser.IsPdfFile());
    }
...
TEST(ParserTest, testRoundTripIndirectTrailerID)
{
    ostringstream oss;
    oss << "%PDF-1.1\n";
    int nCurObj = 0;
